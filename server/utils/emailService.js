const sendOtpEmail = async (to, otp, name) => {
  // If no API key is provided, log an error (prevents crashing if not set up yet)
  if (!process.env.BREVO_API_KEY) {
    console.error("BREVO_API_KEY is not set in environment variables.");
    throw new Error("Email service is not configured correctly.");
  }

  // Fallback to GMAIL_USER if SENDER_EMAIL is not explicitly set
  const senderEmail = process.env.SENDER_EMAIL || process.env.GMAIL_USER;

  if (!senderEmail) {
    console.error("SENDER_EMAIL or GMAIL_USER is not set in environment variables.");
    throw new Error("Sender email is not configured correctly.");
  }
