...
export default function AdminLayout() {
  ...
  const isActive = (itemPath, isExact) => {
    if (isExact) return location.pathname === itemPath;
    return location.pathname.startsWith(itemPath) && itemPath !== '/admin';
  };
      ...
      >
        <div className="p-6 border-b border-white/5 flex justify-between items-center">
          <Link to="/" className="flex items-center gap-2 group">
            ...
            <div>
              <p className="font-display text-white font-bold leading-tight">Admin Panel</p>
      ...
      <main className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        ...
        <header className="h-16 glass border-b border-white/5 flex items-center justify-between px-6 shrink-0">
          ...
          <div className="flex-1 flex justify-end">
             <Link to="/" className="text-sm text-slate-400 hover:text-white transition-colors bg-white/5 px-3 py-1.5 rounded-lg border border-white/10">
               View Live Site ↗
             </Link>
          </div>
        </header>
