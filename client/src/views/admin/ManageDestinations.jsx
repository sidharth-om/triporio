...
const categories = ['Beach', 'Hill Station', 'Wildlife', 'Heritage', 'Culture', 'Nature', 'Adventure', 'Temple'];
...
export default function ManageDestinations() {
  ...
  const [formData, setFormData] = useState({
    ...
    location: '', bestSeason: '', featured: false, highlights: '', rating: 4.5
  });
            ...
            <tbody className="divide-y divide-white/5">
              {destinations.map(dest => (
                <tr key={dest._id} className="hover:bg-white/5 transition-colors">
                  ...
                  <td className="px-6 py-4">⭐ {dest.rating || 4.5}</td>
                  <td className="px-6 py-4">
                    {dest.featured ? (
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input required type="number" step="0.1" min="0" max="5" value={formData.rating} onChange={e => setFormData({...formData, rating: parseFloat(e.target.value) || 0})} className="input-dark w-full" />
                </div>
              </div>
