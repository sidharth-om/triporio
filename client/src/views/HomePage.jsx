...
export default function HomePage() {
  ...
  const statsList = [
    { number: siteStats.destinationsCount, label: 'Destinations', icon: FiMapPin },
    { number: siteStats.happyTravelers, label: 'Happy Travelers', icon: FiUsers },
    { number: siteStats.averageRating, label: 'Average Rating', icon: FiStar },
    { number: siteStats.daysOfBeauty, label: 'Days of Beauty', icon: FiCalendar },
  ];

  useEffect(() => {
    const fetchData = async () => {
      try {
        const [destRes, evtRes, statsRes] = await Promise.all([
          destinationService.getAll(),
          eventService.getAll(),
          destinationService.getStats()
        ]);
        // Filter featured destinations
        setFeatured(destRes.data.destinations.filter(d => d.featured));
        setEvents(evtRes.data.events.slice(0, 3));
        if (statsRes.data?.success) {
          const s = statsRes.data.stats;
          setSiteStats({
            destinationsCount: `${s.destinationsCount}+`,
            happyTravelers: s.happyTravelers >= 1000 ? `${(s.happyTravelers / 1000).toFixed(1)}K+` : `${s.happyTravelers}+`,
    ...
    <div className="min-h-screen">
      ...
      <section className="py-10 border-y border-white/10 bg-[#060b18]">
        <div className="max-w-5xl mx-auto px-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {statsList.map(({ number, label, icon: Icon }, i) => (
              <motion.div
                key={i}
                initial={{ opacity: 0, y: 20 }}
      ...
      <section className="py-20 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-14">
            ...
            >
              Kerala's Magic with <span className="gradient-text">Triporio</span>
            </motion.h2>
          </div>
      ...
      <section className="py-20 px-4 bg-[#060b18]">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-14">
            ...
            <Link href="/destinations" className="btn-secondary mt-4 sm:mt-0">
            ...
            {events.map((event, i) => (
              ...
              >
                ...
                <div className="p-5">
                  ...
                  <Link href="/events" className="inline-flex items-center gap-1 text-amber-400 text-sm font-semibold mt-4 hover:gap-2 transition-all">
      ...
      <section className="py-20 px-4 bg-gradient-to-r from-green-900/30 to-teal-900/30 border-y border-white/10">
        <div className="max-w-4xl mx-auto text-center">
          ...
          >
            Ready to Explore Kerala with <span className="gradient-text">Triporio?</span>
          </motion.h2>
